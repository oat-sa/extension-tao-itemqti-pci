...
define("css!mathEntryInteraction/runtime/css/mathEntryInteraction",[],function(){}),define("mathEntryInteraction/runtime/mathEntryInteraction",["qtiCustomInteractionContext","taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/util/event","mathEntryInteraction/runtime/mathquill/mathquill","mathEntryInteraction/runtime/polyfill/es6-collections","css!mathEntryInteraction/runtime/mathquill/mathquill","css!mathEntryInteraction/runtime/css/mathEntryInteraction"],function(qtiCustomInteractionContext,$,_,event,MathQuill){"use strict";function htmlMarkup(cls,tag){return"<"+(tag=tag||"div")+' class="'+cls+'"></'+tag+">"}
function getWidth(element){var style=element.currentStyle||window.getComputedStyle(element),rect=element.getBoundingClientRect(),borderBox="border-box"===style.boxSizing;return rect.width+parseFloat(style.marginLeft)+parseFloat(style.marginRight)+(borderBox?0:parseFloat(style.paddingLeft)+parseFloat(style.paddingRight))+(borderBox?0:parseFloat(style.borderLeftWidth)+parseFloat(style.borderRightWidth))}var mathEntryInteraction,ns=".mathEntryInteraction",cssClass={root:"mq-root-block",cursor:"mq-cursor",newLine:"mq-tao-br",autoWrap:"mq-tao-wrap"},cssSelectors=_.mapValues(cssClass,function(cls){return"."+cls
}),reSpace=/^(\s|&nbsp;)+$/,MQ=MathQuill.getInterface(2);MQ.registerEmbed("gap",function(){return{htmlString:htmlMarkup("mq-tao-gap","span"),text:function(){return"tao_gap"},latex:function(){return"\\taoGap"}}}),MQ.registerEmbed("br",function(){return{htmlString:htmlMarkup(cssClass.newLine),text:function(){return"tao_br"},latex:function(){return"\\taoBr"}}}),mathEntryInteraction={inQtiCreator:function(){return _.isUndefined(this._inQtiCreator)&&this.$container&&(this._inQtiCreator=this.$container.hasClass("tao-qti-creator-context")),this._inQtiCreator},inGapMode:function(){return this.config.useGapExpression},render:function(config){this.initConfig(config),
this.createToolbar(),this.togglePlaceholder(!1),this.inGapMode()&&this.inQtiCreator()?(this.createMathEditable(),this.setLatex(this.config.gapExpression),this.addToolbarListeners(),this.addGapStyle(),this.autoWrapContent()):!this.inGapMode()&&this.inQtiCreator()?this.togglePlaceholder(!0):this.inGapMode()&&!this.inQtiCreator()?(this.setMathStaticContent(this.config.gapExpression),this.createMathStatic(),this.monitorActiveGapField(),this.addToolbarListeners(),this.addGapStyle(),this.autoWrapContent()):(this.createMathEditable(),this.addToolbarListeners())},initConfig:function(config){function toBoolean(value,defaultValue){
return void 0===value?defaultValue:!0===value||"true"===value}this.config={authorizeWhiteSpace:toBoolean(config.authorizeWhiteSpace,!1),useGapExpression:toBoolean(config.useGapExpression,!1),gapExpression:config.gapExpression||"",gapStyle:config.gapStyle,toolsStatus:{frac:toBoolean(config.tool_frac,!0),sqrt:toBoolean(config.tool_sqrt,!0),exp:toBoolean(config.tool_exp,!0),log:toBoolean(config.tool_log,!0),ln:toBoolean(config.tool_ln,!0),e:toBoolean(config.tool_e,!0),infinity:toBoolean(config.tool_infinity,!0),lbrack:toBoolean(config.tool_lbrack,!0),rbrack:toBoolean(config.tool_rbrack,!0),pi:toBoolean(config.tool_pi,!0),cos:toBoolean(config.tool_cos,!0),
sin:toBoolean(config.tool_sin,!0),lte:toBoolean(config.tool_lte,!0),gte:toBoolean(config.tool_gte,!0),times:toBoolean(config.tool_times,!0),divide:toBoolean(config.tool_divide,!0),plusminus:toBoolean(config.tool_plusminus,!0),angle:toBoolean(config.tool_angle,!0),minus:toBoolean(config.tool_minus,!0),plus:toBoolean(config.tool_plus,!0),equal:toBoolean(config.tool_equal,!0),lower:toBoolean(config.tool_lower,!0),greater:toBoolean(config.tool_greater,!0),subscript:toBoolean(config.tool_subscript,!0),lbrace:toBoolean(config.tool_lbrace,!0),rbrace:toBoolean(config.tool_rbrace,!0),lparen:toBoolean(config.tool_lparen,!0),rparen:toBoolean(config.tool_rparen,!0),
integral:toBoolean(config.tool_integral,!0),timesdot:toBoolean(config.tool_timesdot,!0),triangle:toBoolean(config.tool_triangle,!0),similar:toBoolean(config.tool_similar,!0),paral:toBoolean(config.tool_paral,!0),perp:toBoolean(config.tool_perp,!0),inmem:toBoolean(config.tool_inmem,!0),ninmem:toBoolean(config.tool_ninmem,!0),union:toBoolean(config.tool_union,!0),intersec:toBoolean(config.tool_intersec,!0)},allowNewLine:toBoolean(config.allowNewLine,!1),enableAutoWrap:toBoolean(config.enableAutoWrap,!1)}},getMqConfig:function(){var self=this;return{spaceBehavesLikeTab:!this.config.authorizeWhiteSpace,handlers:{edit:function(mathField){
self.trigger("responseChange",[mathField.latex()]),self.autoWrapContent()},enter:function(mathField){(function(){return self.config.allowNewLine&&(!self.inGapMode()&&!self.inQtiCreator()||self.inGapMode()&&self.inQtiCreator())})()&&mathField.write("\\embed{br}")}}}},togglePlaceholder:function(displayPlaceholder){this.$inputPlaceholder||(this.$inputPlaceholder=$("<div>",{class:"math-entry-placeholder"}),this.$toolbar.after(this.$inputPlaceholder)),displayPlaceholder?(this.$input.hide(),this.$inputPlaceholder.show()):(this.$input.css({display:"block"}),this.$inputPlaceholder.hide())},autoWrapContent:function(){
...
return(className.match(/\bmath-gap-[\w]+\b/g)||[]).join(" ")}),self.$container.addClass(self.config.gapStyle))},setResponse:function(response){this.inGapMode()?response&&response.list&&_.isArray(response.list.string)&&this.setLatex(response.list.string):response&&response.base&&response.base.string&&this.setLatex(response.base.string)},resetResponse:function(){var gapFields=this.getGapFields();this.inGapMode()?gapFields.forEach(function(gapField){gapField.latex("")}):this.setLatex("")},setState:function(state){state&&state.response&&this.setResponse(state.response)},typeIdentifier:"mathEntryInteraction",getInstance:function(dom,config,state){
console.log("getInstance");var self=_.cloneDeep(this);event.addEventMgr(self),self.dom=dom,self.$container=$(dom),self.$toolbar=self.$container.find(".toolbar"),self.$input=self.$container.find(".math-entry-input"),self.render(config.properties),self.on("configChange",function(newConfig){self.render(newConfig)}),self.on("addGap",function(){self.addGap()}),self.setState(state),config.onready(self)},getResponse:function(){return this.inGapMode()?{list:{string:this.getGapFields().map(function(gapField){return gapField.latex()})}}:{base:{string:this.mathField.latex()}}},oncompleted:function(){this.$input.find(".mq-editable-field").off(ns),this.$input.off(ns),
this.$toolbar.off(ns),this.resetResponse(),this.mathField instanceof MathQuill&&this.mathField.revert()},getState:function(){return{response:this.getResponse()}}},qtiCustomInteractionContext.register(mathEntryInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))
