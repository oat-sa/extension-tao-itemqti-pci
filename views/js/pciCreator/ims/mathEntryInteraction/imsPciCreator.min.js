...
helper?helper.call(depth0,"Functions",options):helperMissing.call(depth0,"__","Functions",options)))+'</h3>\r\n\r\n<div>\r\n    <label class="panel">\r\n        <input name="tool_sqrt" type="checkbox" ',stack1=helpers.if.call(depth0,depth0&&depth0.tool_sqrt,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),
...
$gapStyleBox.show()):(i.prop("gapExpression",""),self.removeAddGapBtn(),$gapStyleBox.hide()),response.attr("cardinality","single"),configChangeCallBack(i,value,"useGapExpression")},gapStyle:function(i,newStyle){i.prop("gapStyle",newStyle),configChangeCallBack(i,newStyle,"gapStyle")},authorizeWhiteSpace:configChangeCallBack,focusOnDenominator:configChangeCallBack,tool_frac:configChangeCallBack,tool_sqrt:configChangeCallBack,tool_exp:configChangeCallBack,tool_log:configChangeCallBack,tool_ln:configChangeCallBack,tool_e:configChangeCallBack,tool_infinity:configChangeCallBack,tool_pi:configChangeCallBack,tool_cos:configChangeCallBack,
tool_sin:configChangeCallBack,tool_lte:configChangeCallBack,tool_gte:configChangeCallBack,tool_times:configChangeCallBack,tool_divide:configChangeCallBack,tool_plusminus:configChangeCallBack,tool_angle:configChangeCallBack,tool_minus:configChangeCallBack,tool_plus:configChangeCallBack,tool_equal:configChangeCallBack,tool_lower:configChangeCallBack,tool_greater:configChangeCallBack,tool_subscript:configChangeCallBack,tool_integral:configChangeCallBack,tool_timesdot:configChangeCallBack,tool_triangle:configChangeCallBack,tool_similar:configChangeCallBack,tool_paral:configChangeCallBack,tool_perp:configChangeCallBack,tool_inmem:configChangeCallBack,
tool_ninmem:configChangeCallBack,tool_union:configChangeCallBack,tool_intersec:configChangeCallBack,squarebkts:function(i,value){i.prop("tool_lbrack",value),i.prop("tool_rbrack",value),i.triggerPci("configChange",[i.getProperties()])},roundbkts:function(i,value){i.prop("tool_lparen",value),i.prop("tool_rparen",value),i.triggerPci("configChange",[i.getProperties()])},curlybkts:function(i,value){i.prop("tool_lbrace",value),i.prop("tool_rbrace",value),i.triggerPci("configChange",[i.getProperties()])},allowNewLine:configChangeCallBack,enableAutoWrap:configChangeCallBack}),$gapStyleBox=$form.find(".mathgap-style-box"),
$gapStyleSelector=$gapStyleBox.find("[data-mathgap-style]"),$gapStyleSelector.select2({width:"100%",minimumResultsForSearch:1/0}).val(interaction.prop("gapStyle")).trigger("change")},MathEntryInteractionStateQuestion.prototype.addMathFieldListener=function(){var _widget=this.widget,interaction=_widget.element;interaction.onPci("responseChange",function(latex){toBoolean(interaction.prop("useGapExpression"),!1)?interaction.prop("gapExpression",latex):interaction.prop("gapExpression","")})},MathEntryInteractionStateQuestion.prototype.createAddGapBtn=function(){
...
this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,options,buffer="",functionType="function",escapeExpression=this.escapeExpression,helperMissing=helpers.helperMissing,self=this;return buffer+='<div class="panel mathEntryInteraction">\r\n    <hr>\r\n    <h3>'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
helper?helper.call(depth0,"Define correct responses",options):helperMissing.call(depth0,"__","Define correct responses",options)))+':</h3>\r\n    <div class="answer-entries-config" data-config-for-hot-uid="'+escapeExpression((stack1=null==data||!1===data?data:data.key,typeof stack1===functionType?stack1.apply(depth0):stack1))+'">\r\n        <div class="panel">\r\n            <div class="entry-config">\r\n                ',stack1=helpers.each.call(depth0,depth0&&depth0.correctAnswerEntries,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),
buffer+="\r\n            </div>\r\n        </div>\r\n\r\n    </div>\r\n\r\n</div>"})}),define("tpl!mathEntryInteraction/creator/tpl/addAnswerOption",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,options,buffer="",helperMissing=helpers.helperMissing;return buffer+='<button class="add-answer-option btn-info">\r\n    <i class="adder"> </i> '+(0,this.escapeExpression)((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
helper?helper.call(depth0,"Add answer option",options):helperMissing.call(depth0,"__","Add answer option",options)))+"\r\n</button>"})}),define("mathEntryInteraction/creator/widget/states/Map",["handlebars","i18n","lodash","jquery","taoQtiItem/qtiCreator/widgets/states/factory","taoQtiItem/qtiCreator/widgets/states/Map","taoQtiItem/qtiCreator/widgets/helpers/formElement","tpl!mathEntryInteraction/creator/tpl/answerForm","tpl!mathEntryInteraction/creator/tpl/addAnswerOption"],function(hb,__,_,$,stateFactory,Map,formElement,answerFormTpl,addAnswerOptionBtn){"use strict";hb.registerHelper("increaseIndex",function(value,options){return parseInt(value)+1})
...
var self=this,interaction=self.widget.element;self.widget.$responseForm.find(".entry-config").find(".answer-edit").click(function(e){e.preventDefault(),e.stopImmediatePropagation(),self.toggleResponseMode(!0);var selectedEditId=parseInt($(e.target).closest("div").attr("data-index"));if(self.activeEditId!==selectedEditId)if(!0===self.inGapMode()){self.activeEditId=selectedEditId;var response=self.getGapResponseObject(self.correctResponses[self.activeEditId]);interaction.triggerPci("latexGapInpfut",[response])}else self.activeEditId=parseInt(e.currentTarget.id.split("_")[1]),
...
;self.widget.$responseForm.find(".entry-config").find(".answer-delete").click(function(e){e.preventDefault(),e.stopImmediatePropagation();var id=parseInt($(e.target).closest("div").attr("data-index"));!0===self.inGapMode()?(self.activeEditId=id,self.emptyGapFields()):(self.activeEditId=null,interaction.triggerPci("latexInput",[""])),self.correctResponses.splice(id,1),self.renderForm(self.correctResponses)})},MathEntryInteractionStateResponse.prototype.renderForm=function(correctAnswerOptions){var self=this,$responseForm=self.widget.$responseForm;this.removeEditDeleteListeners(),$responseForm.find(".mathEntryInteraction").remove(),
$responseForm.append(answerFormTpl({correctAnswerEntries:correctAnswerOptions})),this.initDeletingOptions(),this.initEditingOptions()},MathEntryInteractionStateResponse.prototype.removeEditDeleteListeners=function(){var self=this,$entryConfig=self.widget.$responseForm.find(".entry-config");$entryConfig.find(".answer-edit").off("click"),$entryConfig.find(".answer-delete").off("click")},MathEntryInteractionStateResponse.prototype.removeAddButtonListener=function(){this.widget.$responseForm.find($(".add-answer-option")).off("click")},MathEntryInteractionStateResponse.prototype.destroyForm=function(){
this.widget.$responseForm.find(".mathEntryInteraction").remove()},MathEntryInteractionStateResponse.prototype.saveAnswers=function(){var self=this,interaction=self.widget.element,responseDeclaration=interaction.getResponseDeclaration();this.clearMapEntries(),!0===this.inGapMode()&&(self.correctResponses=self.correctResponses.filter(function(response){return-1===response.split(",").indexOf("")})),self.correctResponses.forEach(function(response){responseDeclaration.setMapEntry(response,1,!1)})},MathEntryInteractionStateResponse.prototype.emptyGapFields=function(){var self=this,interaction=self.widget.element;!0===this.inGapMode()&&(self.activeEditId=null,
interaction.prop("gapExpression",self.gapTemplate),this.toggleResponseMode(!1))},MathEntryInteractionStateResponse.prototype.toggleResponseMode=function(value){var self=this,interaction=self.widget.element;interaction.prop("inResponseState")!==value&&(interaction.prop("inResponseState",value),interaction.triggerPci("configChange",[interaction.getProperties()]))},MathEntryInteractionStateResponse.prototype.inGapMode=function(){return this.widget.element.prop("useGapExpression")},MathEntryInteractionStateResponse}),
