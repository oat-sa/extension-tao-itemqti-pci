...
buffer+='/>\r\n        <span class="icon-checkbox"></span>\r\n        '+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"authorize white space",options):helperMissing.call(depth0,"__","authorize white space",options)))+'\r\n    </label>\r\n    <label class="panel">\r\n        <input name="focusOnDenominator" type="checkbox" ',stack1=helpers.if.call(depth0,depth0&&depth0.focusOnDenominator,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),
...
})}),define("mathEntryInteraction/creator/widget/states/Map",["handlebars","i18n","lodash","jquery","taoQtiItem/qtiCreator/widgets/states/factory","taoQtiItem/qtiCreator/widgets/states/Map","taoQtiItem/qtiCreator/widgets/helpers/formElement","tpl!mathEntryInteraction/creator/tpl/answerForm","tpl!mathEntryInteraction/creator/tpl/addAnswerOption"],function(hb,__,_,$,stateFactory,Map,formElement,answerFormTpl,addAnswerOptionBtn){"use strict";function initForm(self){var interaction=self.widget.element,$responseForm=(interaction.getResponseDeclaration(),self.widget.$responseForm);interaction.prop("inResponseState",!0),
interaction.triggerPci("configChange",[interaction.getProperties()]),initResponseChangeEventListener(self),self.correctResponses=getExistingCorrectAnswerOptions(self),$responseForm.html(addAnswerOptionBtn()),initAddAnswerButton(self),renderForm(self,self.correctResponses)}function initAddAnswerButton(self){self.widget.element;self.widget.$responseForm.find($(".add-answer-option")).on("click",function(){var newCorrectAnswer;if(inGapMode(self)){var gapExpression=self.widget.element.prop("gapExpression"),gapCount=(gapExpression.match(/\\taoGap/g)||[]).length;if(gapCount>0){newCorrectAnswer=[];for(var i=0;i<gapCount;i++)newCorrectAnswer.push("")
;newCorrectAnswer=newCorrectAnswer.join(",")}else newCorrectAnswer=""}else newCorrectAnswer="";self.correctResponses.push(newCorrectAnswer),renderForm(self,self.correctResponses)})}function getExistingCorrectAnswerOptions(self){var mapEntries=self.widget.element.getResponseDeclaration().getMapEntries();return _.keys(mapEntries)||[]}function initResponseChangeEventListener(self){var interaction=self.widget.element;self.widget.element.onPci("responseChange",function(latex){if(inGapMode(self)||null==self.activeEditId){if(inGapMode(self)&&null!=self.activeEditId){var response=interaction.getResponse();self.correctResponses[self.activeEditId]=response.base.string
}}else self.correctResponses[self.activeEditId]=latex})}function removeResponseChangeEventListener(self){self.widget.element.offPci("responseChange")}function clearMathFieldLatex(self){var interaction=self.widget.element;inGapMode(self)||interaction.triggerPci("latexInput",[""])}function initEditingOptions(self){var interaction=self.widget.element,$responseForm=self.widget.$responseForm,$entryConfig=$responseForm.find(".entry-config"),$editButtons=$entryConfig.find(".answer-edit");inGapMode(self)?$editButtons.click(function(e){e.preventDefault(),e.stopImmediatePropagation(),self.activeEditId=parseInt(e.currentTarget.id.split("_")[1])
;var response=getGapResponseObject(self.correctResponses[self.activeEditId]);interaction.triggerPci("latexGapInput",[response])}):$editButtons.click(function(e){e.preventDefault(),e.stopImmediatePropagation(),self.activeEditId=parseInt(e.currentTarget.id.split("_")[1]),interaction.triggerPci("latexInput",[self.correctResponses[self.activeEditId]])})}function getGapResponseObject(response){return{list:{string:response.split(",")}}}function clearMapEntries(self){var interaction=self.widget.element,response=interaction.getResponseDeclaration(),mapEntries=response.getMapEntries();_.keys(mapEntries).forEach(function(mapKey){response.removeMapEntry(mapKey,!0)})}
function initDeletingOptions(self){var interaction=self.widget.element;self.widget.$responseForm.find(".entry-config").find(".answer-delete").click(function(e){e.preventDefault(),e.stopImmediatePropagation();var id=parseInt(e.currentTarget.id.split("_")[1]);if(inGapMode(self)){var gapExpression=interaction.prop("gapExpression");self.correctResponses[id].split(",").forEach(function(response){gapExpression=gapExpression.replace(response,"\\taoGap")}),interaction.prop("gapExpression",gapExpression),interaction.triggerPci("configChange",[interaction.getProperties()])}self.activeEditId=null,self.correctResponses.splice(id,1),renderForm(self,self.correctResponses)
})}function renderForm(self,correctAnswerOptions){removeEditDeleteListeners(self);var $responseForm=self.widget.$responseForm;$responseForm.find(".mathEntryInteraction").remove(),$responseForm.append(answerFormTpl({correctAnswerEntries:correctAnswerOptions})),initDeletingOptions(self),initEditingOptions(self)}function removeEditDeleteListeners(self){var $entryConfig=self.widget.$responseForm.find(".entry-config");$entryConfig.find(".answer-edit").off("click"),$entryConfig.find(".answer-delete").off("click")}function removeAddButtonListener(self){self.widget.$responseForm.find($(".add-answer-option")).off("click")}function destroyForm(self){
self.widget.$responseForm.find(".mathEntryInteraction").remove()}function saveAnswers(self){var interaction=self.widget.element,responseDeclaration=interaction.getResponseDeclaration();clearMapEntries(self),self.correctResponses.forEach(function(response){responseDeclaration.setMapEntry(response,CORRECT_ANSWER_VALUE,!1)})}function emptyGapFields(self){var interaction=self.widget.element;if(inGapMode(self)){var gapTemplateString=interaction.prop("gapExpression");if(null!=self.activeEditId){self.correctResponses[self.activeEditId].split(",").forEach(function(responseGap){gapTemplateString=gapTemplateString.replace(responseGap.toString(),"\\taoGap")}),
interaction.prop("gapExpression",gapTemplateString)}interaction.prop("inResponseState",!1),interaction.triggerPci("configChange",[interaction.getProperties()])}}function inGapMode(self){return!!self.widget.element.prop("useGapExpression")}var CORRECT_ANSWER_VALUE=1;return hb.registerHelper("increaseIndex",function(value,options){return parseInt(value)+1}),stateFactory.create(Map,function(){this.activeEditId=null,this.correctResponses=[],initForm(this)},function(){saveAnswers(this),removeResponseChangeEventListener(this),clearMathFieldLatex(this),removeEditDeleteListeners(this),removeAddButtonListener(this),emptyGapFields(this),destroyForm(this)})}),
