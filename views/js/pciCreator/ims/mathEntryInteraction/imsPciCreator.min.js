...
;var gapExpression=interaction.prop("gapExpression"),gapCount=(gapExpression.match(/\\taoGap/g)||[]).length;if(gapCount>0){newCorrectAnswer=[];for(var i=0;i<gapCount;i++)newCorrectAnswer.push(" ");newCorrectAnswer=newCorrectAnswer.join(",")}else newCorrectAnswer=""}else newCorrectAnswer="",self.toggleResponseMode(!1);self.correctResponses.push(newCorrectAnswer),self.renderForm(self.correctResponses)})},MathEntryInteractionStateResponse.prototype.getExistingCorrectAnswerOptions=function(){var self=this,interaction=self.widget.element,mapEntries=interaction.getResponseDeclaration().getMapEntries();return _.keys(mapEntries)||[]},
MathEntryInteractionStateResponse.prototype.initResponseChangeEventListener=function(){var self=this,interaction=self.widget.element;interaction.onPci("responseChange",function(latex){if(!1===self.inGapMode(self)&&null!=self.activeEditId)self.correctResponses[self.activeEditId]=latex;else if(!0===self.inGapMode(self)&&null!=self.activeEditId){var response=interaction.getResponse();null!==response&&(self.correctResponses[self.activeEditId]=response.base.string)}})},MathEntryInteractionStateResponse.prototype.removeResponseChangeEventListener=function(){this.widget.element.offPci("responseChange")},
MathEntryInteractionStateResponse.prototype.initEditingOptions=function(){var self=this,interaction=self.widget.element;self.widget.$responseForm.find(".entry-config").find(".answer-edit").click(function(e){e.preventDefault(),e.stopImmediatePropagation(),self.toggleResponseMode(!0);var selectedEditId=parseInt($(e.target).closest("div").attr("data-index"));if(self.activeEditId!==selectedEditId)if(!0===self.inGapMode()){self.activeEditId=selectedEditId;var response=self.getGapResponseObject(self.correctResponses[self.activeEditId]);interaction.triggerPci("latexGapInput",[response])}else self.activeEditId=selectedEditId,
interaction.triggerPci("latexInput",[self.correctResponses[self.activeEditId]]);else self.emptyGapFields()})},MathEntryInteractionStateResponse.prototype.getGapResponseObject=function(response){return{base:{string:response.split(",")}}},MathEntryInteractionStateResponse.prototype.clearMapEntries=function(){var self=this,interaction=self.widget.element,response=interaction.getResponseDeclaration(),mapEntries=response.getMapEntries();_.keys(mapEntries).forEach(function(mapKey){response.removeMapEntry(mapKey,!0)})},MathEntryInteractionStateResponse.prototype.initDeletingOptions=function(){var self=this;self.widget.element
;self.widget.$responseForm.find(".entry-config").find(".answer-delete").click(function(e){e.preventDefault(),e.stopImmediatePropagation();var id=parseInt($(e.target).closest("div").attr("data-index"));!0===self.inGapMode()?(self.activeEditId=id,self.emptyGapFields()):(self.activeEditId=null,self.toggleResponseMode(!1)),self.correctResponses.splice(id,1),self.renderForm(self.correctResponses)})},MathEntryInteractionStateResponse.prototype.renderForm=function(correctAnswerOptions){var self=this,$responseForm=self.widget.$responseForm;this.removeEditDeleteListeners(),$responseForm.find(".mathEntryInteraction").remove(),$responseForm.append(answerFormTpl({
