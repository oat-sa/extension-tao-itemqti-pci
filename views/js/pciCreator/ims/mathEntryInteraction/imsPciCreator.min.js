...
})}),define("mathEntryInteraction/creator/widget/states/Map",["handlebars","i18n","lodash","jquery","taoQtiItem/qtiCreator/widgets/states/factory","taoQtiItem/qtiCreator/widgets/states/Map","taoQtiItem/qtiCreator/widgets/helpers/formElement","tpl!mathEntryInteraction/creator/tpl/answerForm","tpl!mathEntryInteraction/creator/tpl/addAnswerOption"],function(hb,__,_,$,stateFactory,Map,formElement,answerFormTpl,addAnswerOptionBtn){"use strict";function initForm(self){var interaction=self.widget.element,$responseForm=(interaction.getResponseDeclaration(),self.widget.$responseForm);initResponseChangeEventListener(self),
self.correctResponses=getExistingCorrectAnswerOptions(self),$responseForm.html(addAnswerOptionBtn()),initAddAnswerButton(self),renderForm(self,self.correctResponses)}function initAddAnswerButton(self){self.widget.element;self.widget.$responseForm.find($(".add-answer-option")).on("click",function(){var newCorrectAnswer;if(inGapMode(self)){emptyGapFields(self);var gapExpression=self.widget.element.prop("gapExpression"),gapCount=(gapExpression.match(/\\taoGap/g)||[]).length;if(gapCount>0){newCorrectAnswer=[];for(var i=0;i<gapCount;i++)newCorrectAnswer.push("");newCorrectAnswer=newCorrectAnswer.join(",")}else newCorrectAnswer=""}else newCorrectAnswer=""
;self.correctResponses.push(newCorrectAnswer),renderForm(self,self.correctResponses)})}function getExistingCorrectAnswerOptions(self){var mapEntries=self.widget.element.getResponseDeclaration().getMapEntries();return _.keys(mapEntries)||[]}function initResponseChangeEventListener(self){var interaction=self.widget.element;self.widget.element.onPci("responseChange",function(latex){if(inGapMode(self)||null==self.activeEditId){if(inGapMode(self)&&null!=self.activeEditId){var response=interaction.getResponse();self.correctResponses[self.activeEditId]=response.base.string}}else self.correctResponses[self.activeEditId]=latex})}
function removeResponseChangeEventListener(self){self.widget.element.offPci("responseChange")}function clearMathFieldLatex(self){var interaction=self.widget.element;inGapMode(self)||interaction.triggerPci("latexInput",[""])}function initEditingOptions(self){var interaction=self.widget.element,$responseForm=self.widget.$responseForm,$entryConfig=$responseForm.find(".entry-config"),$editButtons=$entryConfig.find(".answer-edit");inGapMode(self)?$editButtons.click(function(e){e.preventDefault(),e.stopImmediatePropagation(),!1==!!interaction.prop("inResponseState")&&(interaction.prop("inResponseState",!0),
interaction.triggerPci("configChange",[interaction.getProperties()])),self.activeEditId=parseInt(e.currentTarget.id.split("_")[1]);var response=getGapResponseObject(self.correctResponses[self.activeEditId]);interaction.triggerPci("latexGapInput",[response])}):$editButtons.click(function(e){e.preventDefault(),e.stopImmediatePropagation(),self.activeEditId=parseInt(e.currentTarget.id.split("_")[1]),interaction.triggerPci("latexInput",[self.correctResponses[self.activeEditId]])})}function getGapResponseObject(response){return{list:{string:response.split(",")}}}function clearMapEntries(self){
var interaction=self.widget.element,response=interaction.getResponseDeclaration(),mapEntries=response.getMapEntries();_.keys(mapEntries).forEach(function(mapKey){response.removeMapEntry(mapKey,!0)})}function initDeletingOptions(self){self.widget.element;self.widget.$responseForm.find(".entry-config").find(".answer-delete").click(function(e){e.preventDefault(),e.stopImmediatePropagation();var id=parseInt(e.currentTarget.id.split("_")[1]);inGapMode(self)&&(self.activeEditId=id,emptyGapFields(self)),self.activeEditId=null,self.correctResponses.splice(id,1),renderForm(self,self.correctResponses)})}function renderForm(self,correctAnswerOptions){
removeEditDeleteListeners(self);var $responseForm=self.widget.$responseForm;$responseForm.find(".mathEntryInteraction").remove(),$responseForm.append(answerFormTpl({correctAnswerEntries:correctAnswerOptions})),initDeletingOptions(self),initEditingOptions(self)}function removeEditDeleteListeners(self){var $entryConfig=self.widget.$responseForm.find(".entry-config");$entryConfig.find(".answer-edit").off("click"),$entryConfig.find(".answer-delete").off("click")}function removeAddButtonListener(self){self.widget.$responseForm.find($(".add-answer-option")).off("click")}function destroyForm(self){self.widget.$responseForm.find(".mathEntryInteraction").remove()}
function saveAnswers(self){var interaction=self.widget.element,responseDeclaration=interaction.getResponseDeclaration();clearMapEntries(self),inGapMode(self)&&(self.correctResponses=self.correctResponses.filter(function(response){return-1===response.split(",").indexOf("")})),self.correctResponses.forEach(function(response){responseDeclaration.setMapEntry(response,CORRECT_ANSWER_VALUE,!1)})}function emptyGapFields(self){var interaction=self.widget.element;if(inGapMode(self)){var gapTemplateString=interaction.prop("gapExpression");if(null!=self.activeEditId){var responses=self.correctResponses[self.activeEditId].split(",");self.activeEditId=null,
responses.forEach(function(responseGap){gapTemplateString=gapTemplateString.replace(responseGap.toString(),"\\taoGap")}),interaction.prop("gapExpression",gapTemplateString)}interaction.prop("inResponseState",!1),interaction.triggerPci("configChange",[interaction.getProperties()])}}function inGapMode(self){return!!self.widget.element.prop("useGapExpression")}var CORRECT_ANSWER_VALUE=1;return hb.registerHelper("increaseIndex",function(value,options){return parseInt(value)+1}),stateFactory.create(Map,function(){this.activeEditId=null,this.correctResponses=[],initForm(this)},function(){emptyGapFields(this),saveAnswers(this),
removeResponseChangeEventListener(this),clearMathFieldLatex(this),removeEditDeleteListeners(this),removeAddButtonListener(this),destroyForm(this)})}),define("mathEntryInteraction/creator/widget/states/states",["taoQtiItem/qtiCreator/widgets/states/factory","taoQtiItem/qtiCreator/widgets/interactions/customInteraction/states/states","mathEntryInteraction/creator/widget/states/Question","mathEntryInteraction/creator/widget/states/Correct","mathEntryInteraction/creator/widget/states/Map"],function(factory,states){"use strict";return factory.createBundle(states,arguments)}),
...
tool_sqrt:"true",tool_exp:"true",tool_log:"true",tool_ln:"true",tool_e:"true",tool_infinity:"true",tool_lbrack:"true",tool_rbrack:"true",tool_pi:"true",tool_cos:"true",tool_sin:"true",tool_lte:"true",tool_gte:"true",tool_times:"true",tool_divide:"true",tool_plusminus:"true",tool_angle:"true",tool_minus:"true",tool_plus:"true",tool_equal:"true",tool_lower:"true",tool_greater:"true",tool_subscript:"true",tool_lbrace:"true",tool_rbrace:"true",tool_lparen:"true",tool_rparen:"true",tool_integral:"true",tool_timesdot:"true",tool_triangle:"true",tool_similar:"true",tool_paral:"true",tool_perp:"true",tool_inmem:"true",tool_ninmem:"true",tool_union:"true",
tool_intersec:"true",allowNewLine:"false",enableAutoWrap:"false"}},afterCreate:function(pci){},getMarkupTemplate:function(){return markupTpl},getMarkupData:function(pci,defaultData){return defaultData.prompt=pci.data("prompt"),defaultData}}}),define(["mathEntryInteraction/imsPciCreator"],function(IMSPCI){return IMSPCI});
