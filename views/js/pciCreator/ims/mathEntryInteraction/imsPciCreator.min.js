...
options={hash:{},data:data},helper?helper.call(depth0,"≤",options):helperMissing.call(depth0,"__","≤",options)))+" "+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"lower than or equal",options):helperMissing.call(depth0,"__","lower than or equal",options)))+'\n    </label>\n</div>\n<div>\n    <label class="panel">\n        <input name="tool_gte" type="checkbox" ',stack1=helpers.if.call(depth0,depth0&&depth0.tool_gte,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),
...
helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,options,buffer="",functionType="function",escapeExpression=this.escapeExpression,helperMissing=helpers.helperMissing,self=this;return buffer+='<div class="panel mathEntryInteraction">\n    <hr>\n    <h3>'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Define correct responses",options):helperMissing.call(depth0,"__","Define correct responses",options)))+':</h3>\n    <div class="answer-entries-config" data-config-for-hot-uid="'+escapeExpression((stack1=null==data||!1===data?data:data.key,
typeof stack1===functionType?stack1.apply(depth0):stack1))+'">\n        <div class="panel">\n            <div class="entry-config">\n                ',stack1=helpers.each.call(depth0,depth0&&depth0.correctAnswerEntries,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n            </div>\n        </div>\n\n    </div>\n\n</div>"})}),define("tpl!mathEntryInteraction/creator/tpl/addAnswerOption",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{}
;var helper,options,buffer="",helperMissing=helpers.helperMissing;return buffer+='<button class="add-answer-option btn-info">\n    <i class="adder"> </i> '+(0,this.escapeExpression)((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Add answer option",options):helperMissing.call(depth0,"__","Add answer option",options)))+"\n</button>"})}),
...
;newCorrectAnswer=newCorrectAnswer.join(",")}else newCorrectAnswer=""}else newCorrectAnswer="",self.toggleResponseMode(!1);self.correctResponses.push(newCorrectAnswer),self.renderForm(self.correctResponses)})},MathEntryInteractionStateResponse.prototype.getExistingCorrectAnswerOptions=function(){var self=this,interaction=self.widget.element,mapEntries=interaction.getResponseDeclaration().getMapEntries();return _.keys(mapEntries)||[]},MathEntryInteractionStateResponse.prototype.initResponseChangeEventListener=function(){var self=this,interaction=self.widget.element;interaction.onPci("responseChange",function(latex){
if(self.inGapMode(self)||null==self.activeEditId){if(self.inGapMode(self)&&null!=self.activeEditId){var response=interaction.getResponse();self.correctResponses[self.activeEditId]=response.base.string}}else self.correctResponses[self.activeEditId]=latex})},MathEntryInteractionStateResponse.prototype.removeResponseChangeEventListener=function(){this.widget.element.offPci("responseChange")},MathEntryInteractionStateResponse.prototype.initEditingOptions=function(){var self=this,interaction=self.widget.element;self.widget.$responseForm.find(".entry-config").find(".answer-edit").click(function(e){e.preventDefault(),e.stopImmediatePropagation(),
self.toggleResponseMode(!0);var selectedEditId=parseInt($(e.target).closest("div").attr("data-index"));if(self.activeEditId!==selectedEditId)if(!0===self.inGapMode()){self.activeEditId=selectedEditId;var response=self.getGapResponseObject(self.correctResponses[self.activeEditId]);interaction.triggerPci("latexGapInput",[response])}else self.activeEditId=selectedEditId,interaction.triggerPci("latexInput",[self.correctResponses[self.activeEditId]]);else self.emptyGapFields()})},MathEntryInteractionStateResponse.prototype.getGapResponseObject=function(response){return{list:{string:response.split(",")}}},
MathEntryInteractionStateResponse.prototype.clearMapEntries=function(){var self=this,interaction=self.widget.element,response=interaction.getResponseDeclaration(),mapEntries=response.getMapEntries();_.keys(mapEntries).forEach(function(mapKey){response.removeMapEntry(mapKey,!0)})},MathEntryInteractionStateResponse.prototype.initDeletingOptions=function(){var self=this;self.widget.element;self.widget.$responseForm.find(".entry-config").find(".answer-delete").click(function(e){e.preventDefault(),e.stopImmediatePropagation();var id=parseInt($(e.target).closest("div").attr("data-index"));!0===self.inGapMode()?(self.activeEditId=id,
self.emptyGapFields()):(self.activeEditId=null,self.toggleResponseMode(!1)),self.correctResponses.splice(id,1),self.renderForm(self.correctResponses)})},MathEntryInteractionStateResponse.prototype.renderForm=function(correctAnswerOptions){var self=this,$responseForm=self.widget.$responseForm;this.removeEditDeleteListeners(),$responseForm.find(".mathEntryInteraction").remove(),$responseForm.append(answerFormTpl({correctAnswerEntries:correctAnswerOptions})),this.initDeletingOptions(),this.initEditingOptions()},MathEntryInteractionStateResponse.prototype.removeEditDeleteListeners=function(){
var self=this,$entryConfig=self.widget.$responseForm.find(".entry-config");$entryConfig.find(".answer-edit").off("click"),$entryConfig.find(".answer-delete").off("click")},MathEntryInteractionStateResponse.prototype.removeAddButtonListener=function(){this.widget.$responseForm.find($(".add-answer-option")).off("click")},MathEntryInteractionStateResponse.prototype.destroyForm=function(){this.widget.$responseForm.find(".mathEntryInteraction").remove()},MathEntryInteractionStateResponse.prototype.saveAnswers=function(){var self=this,interaction=self.widget.element,responseDeclaration=interaction.getResponseDeclaration();this.clearMapEntries(),
!0===this.inGapMode()&&(self.correctResponses=self.correctResponses.filter(function(response){return-1===response.split(",").indexOf("")})),self.correctResponses.forEach(function(response){responseDeclaration.setMapEntry(response,1,!1)})},MathEntryInteractionStateResponse.prototype.emptyGapFields=function(){var self=this,interaction=self.widget.element;!0===this.inGapMode()&&(self.activeEditId=null,interaction.prop("gapExpression",self.gapTemplate),this.toggleResponseMode(!1))},MathEntryInteractionStateResponse.prototype.toggleResponseMode=function(value){var self=this,interaction=self.widget.element
;interaction.prop("inResponseState")!==value&&(interaction.prop("inResponseState",value),interaction.triggerPci("configChange",[interaction.getProperties()]))},MathEntryInteractionStateResponse.prototype.inGapMode=function(){return this.widget.element.prop("useGapExpression")},MathEntryInteractionStateResponse}),
