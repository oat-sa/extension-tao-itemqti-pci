...
math.render($container),tooltip.render($container)}}}),define("likerNavigateLock/runtime/js/renderer",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/OAT/util/html"],function($,html){"use strict";function renderChoices(id,$container,config){var $li,level=parseInt(config.level)||5,$ul=$container.find("ul.likert");$ul.empty();for(var i=1;i<=level;i++)$li=$("<li>",{class:"likert"}),$li.append($("<input>",{type:"radio",name:id,value:i})),$ul.append($li)}function renderLabels(id,$container,config,assetManager){var $ul=$container.find("ul.likert"),$labelMin=$("<span>",{class:"likert-label likert-label-min"
}).html(config["label-min"]),$labelMax=$("<span>",{class:"likert-label likert-label-max"}).html(config["label-max"]);$labelMin.append($("<img>",{src:assetManager.resolve("likerNavigateLock/runtime/assets/ThumbDown.png"),class:"likert-image-append"})),$labelMax.prepend($("<img>",{src:assetManager.resolve("likerNavigateLock/runtime/assets/ThumbUp.png"),class:"likert-image-prepend"})),$ul.before($labelMin),$ul.after($labelMax)}return{render:function(id,container,config,assetManager){var $container=$(container);renderChoices(id,$container,config),renderLabels(id,$container,config,assetManager),html.render($container.find(".prompt"))},
renderChoices:function(id,container,config){renderChoices(id,$(container),config)}}}),define("likerNavigateLock/runtime/js/likerNavigateLock",["qtiCustomInteractionContext","taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/OAT/util/event","likerNavigateLock/runtime/js/renderer"],function(qtiCustomInteractionContext,$,event,renderer){"use strict";var likerNavigateLock={id:-1,getTypeIdentifier:function(){return"likerNavigateLock"},initialize:function(id,dom,config,assetManager){var self=this;this.id=id,this.dom=dom,this.config=config||{},event.addEventMgr(this),renderer.render(this.id,this.dom,this.config,assetManager),
...
state&&state.response&&this.setResponse(state.response)},getSerializedState:function(){return{response:this.getResponse()}}};qtiCustomInteractionContext.register(likerNavigateLock)}),define(["likerNavigateLock/runtime/js/likerNavigateLock"],function(PCI){return PCI});
//# sourceMappingURL=likerNavigateLock.min.js.map