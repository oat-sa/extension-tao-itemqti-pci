...
gte:toBoolean(config.tool_gte,!0),times:toBoolean(config.tool_times,!0),divide:toBoolean(config.tool_divide,!0),plusminus:toBoolean(config.tool_plusminus,!0)},allowNewLine:toBoolean(config.allowNewLine,!1)}},getMqConfig:function(){var self=this;return{spaceBehavesLikeTab:!this.config.authorizeWhiteSpace,handlers:{edit:function(mathField){self.trigger("responseChange",[mathField.latex()])},enter:function(mathField){self.config.allowNewLine&&!self.inGapMode()&&mathField.write("\\embed{br}")}}}},togglePlaceholder:function(displayPlaceholder){this.$inputPlaceholder||(this.$inputPlaceholder=$("<div>",{class:"math-entry-placeholder"}),this.$toolbar.after(this.$inputPlaceholder)),
displayPlaceholder?(this.$input.hide(),this.$inputPlaceholder.show()):(this.$input.css({display:"block"}),this.$inputPlaceholder.hide())},setMathStaticContent:function(latex){latex=latex.replace(/\\taoGap/g,"\\MathQuillMathField{}"),this.$input.text(latex)},createMathStatic:function(){var gapFields,self=this;this.mathField=MQ.StaticMath(this.$input.get(0)),gapFields=this.getGapFields(),gapFields.forEach(function(field){field.config(self.getMqConfig())})},monitorActiveGapField:function(){var self=this,$editableFields=this.$input.find(".mq-editable-field");this._activeGapFieldIndex=null,$editableFields.length&&$editableFields.each(function(index){$(this).off(ns).on("click"+ns+" keyup"+ns,function(){
self._activeGapFieldIndex=index})})},createMathEditable:function(){var config=this.getMqConfig();this.mathField&&this.mathField instanceof MathQuill?this.mathField.config(config):this.mathField=MQ.MathField(this.$input.get(0),config)},setLatex:function(latex){var gapFields;this.inGapMode()&&_.isArray(latex)?(gapFields=this.getGapFields(),latex.forEach(function(latexExpr,i){gapFields[i]&&gapFields[i].latex(latexExpr)})):(latex=latex.replace(/\\taoGap/g,"\\embed{gap}"),this.mathField.latex(latex))},insertLatex:function(latex,fn){var activeMathField=this.getActiveMathField();activeMathField&&_.isFunction(activeMathField[fn])&&(activeMathField[fn](latex),activeMathField.focus());
},getActiveMathField:function(){var activeMathField,gapFields;return this.inGapMode()&&!this.inQtiCreator()?(_.isFinite(this._activeGapFieldIndex)||(this._activeGapFieldIndex=0),gapFields=this.getGapFields(),gapFields.length>0&&(activeMathField=gapFields[this._activeGapFieldIndex])):activeMathField=this.mathField,activeMathField},getGapFields:function(){return this.mathField&&_.isArray(this.mathField.innerFields)?this.mathField.innerFields:[]},addGap:function(){this.inQtiCreator()&&this.insertLatex("\\embed{gap}","write")},createToolbar:function(){var self=this,availableTools={frac:{label:"x/y",latex:"\\frac",fn:"cmd",desc:"Fraction"},sqrt:{label:"&radic;",latex:"\\sqrt",
fn:"cmd",desc:"Square root"},exp:{label:"x&#8319;",latex:"^",fn:"cmd",desc:"Exponent"},log:{label:"log",latex:"\\log",fn:"cmd",desc:"Log"},ln:{label:"ln",latex:"\\ln",fn:"cmd",desc:"Ln"},e:{label:"&#8494;",latex:"\\mathrm{e}",fn:"write",desc:"Euler's constant"},infinity:{label:"&#8734;",latex:"\\infty",fn:"cmd",desc:"Infinity"},lbrack:{label:"[",latex:"\\lbrack",fn:"cmd",desc:"Left bracket"},rbrack:{label:"]",latex:"\\rbrack",fn:"cmd",desc:"Right bracket"},pi:{label:"&pi;",latex:"\\pi",fn:"cmd",desc:"Pi"},cos:{label:"cos",latex:"\\cos",fn:"cmd",desc:"Cosinus"},sin:{label:"sin",latex:"\\sin",fn:"cmd",desc:"Sinus"},lte:{label:"&le;",latex:"\\le",fn:"cmd",desc:"Lower than or equal"
},gte:{label:"&ge;",latex:"\\ge",fn:"cmd",desc:"Greater than or equal"},times:{label:"&times;",latex:"\\times",fn:"cmd",desc:"Multiply"},divide:{label:"&divide;",latex:"\\div",fn:"cmd",desc:"Divide"},plusminus:{label:"&#177;",latex:"\\pm",fn:"cmd",desc:"Plus/minus"}},availableToolGroups=[{id:"functions",tools:["sqrt","frac","exp","log","ln"]},{id:"symbols",tools:["e","infinity","lbrack","rbrack"]},{id:"trigo",tools:["pi","sin","cos"]},{id:"comparison",tools:["lte","gte"]},{id:"operands",tools:["times","divide","plusminus"]}];this.$toolbar.empty(),availableToolGroups.forEach(function(toolgroup){self.$toolbar.append(self.createToolGroup(toolgroup,availableTools));
})},createToolGroup:function(group,availableTools){var self=this,$toolGroup=$("<div>",{class:"math-entry-toolgroup","data-identifier":group.id}),activeTools=0;return group.tools.forEach(function(toolId){var toolConfig=availableTools[toolId];toolConfig.id=toolId,self.config.toolsStatus[toolId]===!0&&($toolGroup.append(self.createTool(toolConfig)),activeTools++)}),activeTools>0?$toolGroup:""},createTool:function(config){return $("<div>",{class:"math-entry-tool","data-identifier":config.id,"data-latex":config.latex,"data-fn":config.fn,html:config.label})},addToolbarListeners:function(){var self=this;this.$toolbar.off("mousedown"+ns).on("mousedown"+ns,function(e){
var $target=$(e.target),fn=$target.data("fn"),latex=$target.data("latex");e.stopPropagation(),e.preventDefault(),self.insertLatex(latex,fn)})},addGapStyle:function(){var self=this;self.config.gapStyle&&(self.$container.removeClass(function(index,className){return(className.match(/\bmath-gap-[\w]+\b/g)||[]).join(" ")}),self.$container.addClass(self.config.gapStyle))},id:-1,getTypeIdentifier:function(){return"mathEntryInteraction"},initialize:function(id,dom,config){var self=this;event.addEventMgr(this),this.id=id,this.dom=dom,this.$container=$(dom),this.$toolbar=this.$container.find(".toolbar"),this.$input=this.$container.find(".math-entry-input"),this.render(config),
this.on("configChange",function(newConfig){self.render(newConfig)}),this.on("addGap",function(){self.addGap()}),html.render(this.$container.find(".prompt")),qtiCustomInteractionContext.notifyReady(this)},setResponse:function(response){this.inGapMode()?response&&response.list&&_.isArray(response.list.string)&&this.setLatex(response.list.string):response&&response.base&&response.base.string&&this.setLatex(response.base.string)},getResponse:function(){var response;return response=this.inGapMode()?{list:{string:this.getGapFields().map(function(gapField){return gapField.latex()})}}:{base:{string:this.mathField.latex()}}},resetResponse:function(){var gapFields=this.getGapFields();
