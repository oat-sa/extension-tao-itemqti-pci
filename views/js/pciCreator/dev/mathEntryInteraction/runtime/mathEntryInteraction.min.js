...
"use strict";var mathEntryInteraction={render:function(config){this.initConfig(config),this.createToolbar(),this.createMathField()},initConfig:function(config){function toBoolean(value,defaultValue){return"undefined"==typeof value?defaultValue:value===!0||"true"===value}this.config={toolsStatus:{frac:toBoolean(config.tool_frac,!0),sqrt:toBoolean(config.tool_sqrt,!0),exp:toBoolean(config.tool_exp,!0),log:toBoolean(config.tool_log,!0),ln:toBoolean(config.tool_ln,!0),e:toBoolean(config.tool_e,!0),infinity:toBoolean(config.tool_infinity,!0),lbrack:toBoolean(config.tool_lbrack,!0),rbrack:toBoolean(config.tool_rbrack,!0),pi:toBoolean(config.tool_pi,!0),cos:toBoolean(config.tool_cos,!0),
sin:toBoolean(config.tool_sin,!0),lte:toBoolean(config.tool_lte,!0),gte:toBoolean(config.tool_gte,!0),times:toBoolean(config.tool_times,!0),divide:toBoolean(config.tool_divide,!0),plusminus:toBoolean(config.tool_plusminus,!0)},allowNewLine:toBoolean(config.allowNewLine,!1),authorizeWhiteSpace:toBoolean(config.authorizeWhiteSpace,!1)}},createMathField:function(){var self=this,MQ=MathQuill.getInterface(2),config={spaceBehavesLikeTab:!this.config.authorizeWhiteSpace,handlers:{edit:function(){self.trigger("responseChange")}}};this.mathField&&this.mathField instanceof MathQuill?this.mathField.config(config):this.mathField=MQ.MathField(this.$input.get(0),config)},createToolbar:function(){
function createToolGroup(group){var $toolGroup=$("<div>",{class:"math-entry-toolgroup","data-identifier":group.id}),activeTools=0;return group.tools.forEach(function(toolId){var toolConfig=availableTools[toolId];toolConfig.id=toolId,self.config.toolsStatus[toolId]===!0&&($toolGroup.append(createTool(toolConfig)),activeTools++)}),activeTools>0?$toolGroup:""}function createTool(config){return $("<div>",{class:"math-entry-tool","data-identifier":config.id,"data-latex":config.latex,"data-fn":config.fn,html:config.label})}var self=this,availableTools={frac:{label:"x/y",latex:"\\frac",fn:"cmd",desc:"Fraction"},sqrt:{label:"&radic;",latex:"\\sqrt",fn:"cmd",desc:"Square root"
},exp:{label:"x&#8319;",latex:"^",fn:"cmd",desc:"Exponent"},log:{label:"log",latex:"\\log",fn:"write",desc:"Log"},ln:{label:"ln",latex:"\\ln",fn:"write",desc:"Ln"},e:{label:"&#8494;",latex:"\\mathrm{e}",fn:"write",desc:"Euler's constant"},infinity:{label:"&#8734;",latex:"\\infty",fn:"write",desc:"Infinity"},lbrack:{label:"[",latex:"\\lbrack",fn:"write",desc:"Left bracket"},rbrack:{label:"]",latex:"\\rbrack",fn:"write",desc:"Right bracket"},pi:{label:"&pi;",latex:"\\pi",fn:"write",desc:"Pi"},cos:{label:"cos",latex:"\\cos",fn:"write",desc:"Cosinus"},sin:{label:"sin",latex:"\\sin",fn:"write",desc:"Sinus"},lte:{label:"&le;",latex:"\\le",fn:"write",desc:"Lower than or equal"
},gte:{label:"&ge;",latex:"\\ge",fn:"write",desc:"Greater than or equal"},times:{label:"&times;",latex:"\\times",fn:"cmd",desc:"Multiply"},divide:{label:"&divide;",latex:"\\div",fn:"cmd",desc:"Divide"},plusminus:{label:"&#177;",latex:"\\pm",fn:"write",desc:"Plus/minus"}},availableToolGroups=[{id:"functions",tools:["sqrt","frac","exp","log","ln"]},{id:"symbols",tools:["e","infinity","lbrack","rbrack"]},{id:"trigo",tools:["pi","sin","cos"]},{id:"comparison",tools:["lte","gte"]},{id:"operands",tools:["times","divide","plusminus"]}];this.$toolbar.empty(),availableToolGroups.forEach(function(toolgroup){self.$toolbar.append(createToolGroup(toolgroup))}),this.$toolbar.off("mousedown.qtiCommonRenderer"),
this.$toolbar.on("mousedown.qtiCommonRenderer",function(e){var $target=$(e.target),fn=$target.data("fn"),latex=$target.data("latex");switch(e.stopPropagation(),e.preventDefault(),fn){case"cmd":self.mathField.cmd(latex);break;case"write":self.mathField.write(latex)}self.mathField.focus()}),this.$input.on("keypress.qtiCommonRenderer",function(e){var latex="\\textcolor{newline}{ }";self.config.allowNewLine&&13===e.keyCode&&self.mathField.write(latex)})},id:-1,getTypeIdentifier:function(){return"mathEntryInteraction"},initialize:function(id,dom,config){var self=this;event.addEventMgr(this),this.id=id,this.dom=dom,this.$container=$(dom),this.$toolbar=this.$container.find(".toolbar"),
this.$input=this.$container.find(".math-entry-input"),this.render(config),qtiCustomInteractionContext.notifyReady(this),this.on("configChange",function(newConfig){self.render(newConfig)}),html.render(this.$container.find(".prompt"))},setResponse:function(response){response&&response.base&&response.base.string&&this.mathField.latex(response.base.string)},getResponse:function(){return{base:{string:this.mathField.latex()}}},resetResponse:function(){this.mathField.latex("")},destroy:function(){this.$toolbar.off("mousedown.qtiCommonRenderer"),this.$input.off("keypress.qtiCommonRenderer"),this.resetResponse(),this.mathField.revert()},setSerializedState:function(state){
this.setResponse(state)},getSerializedState:function(){return this.getResponse()}};qtiCustomInteractionContext.register(mathEntryInteraction)}),define(["mathEntryInteraction/runtime/mathEntryInteraction"],function(PCI){return PCI});
