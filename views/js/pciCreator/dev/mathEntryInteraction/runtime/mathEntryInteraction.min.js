...
return string("[").then(regex(/^[-\w\s]*/)).skip(string("]")).or(succeed()).map(function(data){return self.setOptions(EMBEDS[name](data))})})}}),MQ1=getInterface(1);for(var key in MQ1)(function(key,val){"function"==typeof val?(MathQuill[key]=function(){return insistOnInterVer(),val.apply(this,arguments)},MathQuill[key].prototype=val.prototype):MathQuill[key]=val})(key,MQ1[key]);return MathQuill}),define("mathEntryInteraction/runtime/mathEntryInteraction",["qtiCustomInteractionContext","taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/OAT/util/html","mathEntryInteraction/runtime/mathquill/mathquill"],function(qtiCustomInteractionContext,$,_,event,html,MathQuill){
"use strict";function htmlMarkup(cls,tag){return tag=tag||"div","<"+tag+' class="'+cls+'"></'+tag+">"}function getWidth(element){var style=element.currentStyle||window.getComputedStyle(element),rect=element.getBoundingClientRect(),borderBox="border-box"===style.boxSizing;return rect.width+parseFloat(style.marginLeft)+parseFloat(style.marginRight)+(borderBox?0:parseFloat(style.paddingLeft)+parseFloat(style.paddingRight))+(borderBox?0:parseFloat(style.borderLeftWidth)+parseFloat(style.borderRightWidth))}function applyAutoWrap($container){var maxWidth=$container.width(),lineWidth=0;$container.find(autoWrapSelector).remove(),$container.children().each(function(){
var $block=$(this),width=getWidth(this);this.classList.contains(newLineSelector)?lineWidth=0:(lineWidth+width>=maxWidth&&($block.before(htmlMarkup(autoWrapClass)),lineWidth=0),lineWidth+=width)})}var mathEntryInteraction,ns=".mathEntryInteraction",rootClass="mq-root-block",newLineClass="mq-tao-br",autoWrapClass="mq-tao-wrap",rootSelector="."+rootClass,newLineSelector="."+newLineClass,autoWrapSelector="."+autoWrapClass,MQ=MathQuill.getInterface(2);MQ.registerEmbed("gap",function(){return{htmlString:htmlMarkup("mq-tao-gap","span"),text:function(){return"tao_gap"},latex:function(){return"\\taoGap"}}}),MQ.registerEmbed("br",function(){return{htmlString:htmlMarkup(newLineClass),
text:function(){return"tao_br"},latex:function(){return"\\taoBr"}}}),mathEntryInteraction={inQtiCreator:function(){return _.isUndefined(this._inQtiCreator)&&this.$container&&(this._inQtiCreator=this.$container.hasClass("tao-qti-creator-context")),this._inQtiCreator},inGapMode:function(){return this.config.useGapExpression},render:function(config){this.initConfig(config),this.createToolbar(),this.togglePlaceholder(!1),this.inGapMode()&&this.inQtiCreator()?(this.createMathEditable(),this.setLatex(this.config.gapExpression),this.addToolbarListeners(),this.addGapStyle()):!this.inGapMode()&&this.inQtiCreator()?this.togglePlaceholder(!0):this.inGapMode()&&!this.inQtiCreator()?(this.setMathStaticContent(this.config.gapExpression),
this.createMathStatic(),this.monitorActiveGapField(),this.addToolbarListeners(),this.addGapStyle()):(this.createMathEditable(),this.addToolbarListeners())},initConfig:function(config){function toBoolean(value,defaultValue){return"undefined"==typeof value?defaultValue:value===!0||"true"===value}this.config={authorizeWhiteSpace:toBoolean(config.authorizeWhiteSpace,!1),useGapExpression:toBoolean(config.useGapExpression,!1),gapExpression:config.gapExpression||"",gapStyle:config.gapStyle,toolsStatus:{frac:toBoolean(config.tool_frac,!0),sqrt:toBoolean(config.tool_sqrt,!0),exp:toBoolean(config.tool_exp,!0),log:toBoolean(config.tool_log,!0),ln:toBoolean(config.tool_ln,!0),
e:toBoolean(config.tool_e,!0),infinity:toBoolean(config.tool_infinity,!0),lbrack:toBoolean(config.tool_lbrack,!0),rbrack:toBoolean(config.tool_rbrack,!0),pi:toBoolean(config.tool_pi,!0),cos:toBoolean(config.tool_cos,!0),sin:toBoolean(config.tool_sin,!0),lte:toBoolean(config.tool_lte,!0),gte:toBoolean(config.tool_gte,!0),times:toBoolean(config.tool_times,!0),divide:toBoolean(config.tool_divide,!0),plusminus:toBoolean(config.tool_plusminus,!0)},allowNewLine:toBoolean(config.allowNewLine,!1),allowAutoWrap:toBoolean(config.allowAutoWrap,!1)}},getMqConfig:function(){var self=this;return{spaceBehavesLikeTab:!this.config.authorizeWhiteSpace,handlers:{edit:function(mathField){
self.config.allowAutoWrap&&applyAutoWrap(self.$input.find(rootSelector)),self.trigger("responseChange",[mathField.latex()])},enter:function(mathField){function isBrAllowed(){return self.config.allowNewLine&&(!self.inGapMode()&&!self.inQtiCreator()||self.inGapMode()&&self.inQtiCreator())}isBrAllowed()&&mathField.write("\\embed{br}")}}}},togglePlaceholder:function(displayPlaceholder){this.$inputPlaceholder||(this.$inputPlaceholder=$("<div>",{class:"math-entry-placeholder"}),this.$toolbar.after(this.$inputPlaceholder)),displayPlaceholder?(this.$input.hide(),this.$inputPlaceholder.show()):(this.$input.css({display:"block"}),this.$inputPlaceholder.hide())},setMathStaticContent:function(latex){
latex=latex.replace(/\\taoGap/g,"\\MathQuillMathField{}").replace(/\\taoBr/g,"\\embed{br}"),this.$input.text(latex)},createMathStatic:function(){var gapFields,self=this;this.mathField=MQ.StaticMath(this.$input.get(0)),gapFields=this.getGapFields(),gapFields.forEach(function(field){field.config(self.getMqConfig())})},monitorActiveGapField:function(){var self=this,$editableFields=this.$input.find(".mq-editable-field");this._activeGapFieldIndex=null,$editableFields.length&&$editableFields.each(function(index){$(this).off(ns).on("click"+ns+" keyup"+ns,function(){self._activeGapFieldIndex=index})})},createMathEditable:function(){var config=this.getMqConfig();this.mathField&&this.mathField instanceof MathQuill?this.mathField.config(config):this.mathField=MQ.MathField(this.$input.get(0),config);
},setLatex:function(latex){var gapFields;this.inGapMode()&&_.isArray(latex)?(gapFields=this.getGapFields(),latex.forEach(function(latexExpr,i){gapFields[i]&&gapFields[i].latex(latexExpr)})):(latex=latex.replace(/\\taoGap/g,"\\embed{gap}").replace(/\\taoBr/g,"\\embed{br}"),this.mathField.latex(latex))},insertLatex:function(latex,fn){var activeMathField=this.getActiveMathField();activeMathField&&_.isFunction(activeMathField[fn])&&(activeMathField[fn](latex),activeMathField.focus())},getActiveMathField:function(){var activeMathField,gapFields;return this.inGapMode()&&!this.inQtiCreator()?(_.isFinite(this._activeGapFieldIndex)||(this._activeGapFieldIndex=0),gapFields=this.getGapFields(),
gapFields.length>0&&(activeMathField=gapFields[this._activeGapFieldIndex])):activeMathField=this.mathField,activeMathField},getGapFields:function(){return this.mathField&&_.isArray(this.mathField.innerFields)?this.mathField.innerFields:[]},addGap:function(){this.inQtiCreator()&&this.insertLatex("\\embed{gap}","write")},createToolbar:function(){var self=this,availableTools={frac:{label:"x/y",latex:"\\frac",fn:"cmd",desc:"Fraction"},sqrt:{label:"&radic;",latex:"\\sqrt",fn:"cmd",desc:"Square root"},exp:{label:"x&#8319;",latex:"^",fn:"cmd",desc:"Exponent"},log:{label:"log",latex:"\\log",fn:"cmd",desc:"Log"},ln:{label:"ln",latex:"\\ln",fn:"cmd",desc:"Ln"},e:{label:"&#8494;",
latex:"\\mathrm{e}",fn:"write",desc:"Euler's constant"},infinity:{label:"&#8734;",latex:"\\infty",fn:"cmd",desc:"Infinity"},lbrack:{label:"[",latex:"\\lbrack",fn:"cmd",desc:"Left bracket"},rbrack:{label:"]",latex:"\\rbrack",fn:"cmd",desc:"Right bracket"},pi:{label:"&pi;",latex:"\\pi",fn:"cmd",desc:"Pi"},cos:{label:"cos",latex:"\\cos",fn:"cmd",desc:"Cosinus"},sin:{label:"sin",latex:"\\sin",fn:"cmd",desc:"Sinus"},lte:{label:"&le;",latex:"\\le",fn:"cmd",desc:"Lower than or equal"},gte:{label:"&ge;",latex:"\\ge",fn:"cmd",desc:"Greater than or equal"},times:{label:"&times;",latex:"\\times",fn:"cmd",desc:"Multiply"},divide:{label:"&divide;",latex:"\\div",fn:"cmd",desc:"Divide"
},plusminus:{label:"&#177;",latex:"\\pm",fn:"cmd",desc:"Plus/minus"}},availableToolGroups=[{id:"functions",tools:["sqrt","frac","exp","log","ln"]},{id:"symbols",tools:["e","infinity","lbrack","rbrack"]},{id:"trigo",tools:["pi","sin","cos"]},{id:"comparison",tools:["lte","gte"]},{id:"operands",tools:["times","divide","plusminus"]}];this.$toolbar.empty(),availableToolGroups.forEach(function(toolgroup){self.$toolbar.append(self.createToolGroup(toolgroup,availableTools))})},createToolGroup:function(group,availableTools){var self=this,$toolGroup=$("<div>",{class:"math-entry-toolgroup","data-identifier":group.id}),activeTools=0;return group.tools.forEach(function(toolId){
var toolConfig=availableTools[toolId];toolConfig.id=toolId,self.config.toolsStatus[toolId]===!0&&($toolGroup.append(self.createTool(toolConfig)),activeTools++)}),activeTools>0?$toolGroup:""},createTool:function(config){return $("<div>",{class:"math-entry-tool","data-identifier":config.id,"data-latex":config.latex,"data-fn":config.fn,html:config.label})},addToolbarListeners:function(){var self=this;this.$toolbar.off("mousedown"+ns).on("mousedown"+ns,function(e){var $target=$(e.target),fn=$target.data("fn"),latex=$target.data("latex");e.stopPropagation(),e.preventDefault(),self.insertLatex(latex,fn)})},addGapStyle:function(){var self=this;self.config.gapStyle&&(self.$container.removeClass(function(index,className){
return(className.match(/\bmath-gap-[\w]+\b/g)||[]).join(" ")}),self.$container.addClass(self.config.gapStyle))},id:-1,getTypeIdentifier:function(){return"mathEntryInteraction"},initialize:function(id,dom,config){var self=this;event.addEventMgr(this),this.id=id,this.dom=dom,this.$container=$(dom),this.$toolbar=this.$container.find(".toolbar"),this.$input=this.$container.find(".math-entry-input"),this.render(config),this.on("configChange",function(newConfig){self.render(newConfig)}),this.on("addGap",function(){self.addGap()}),html.render(this.$container.find(".prompt")),qtiCustomInteractionContext.notifyReady(this)},setResponse:function(response){this.inGapMode()?response&&response.list&&_.isArray(response.list.string)&&this.setLatex(response.list.string):response&&response.base&&response.base.string&&this.setLatex(response.base.string);
},getResponse:function(){var response;return response=this.inGapMode()?{list:{string:this.getGapFields().map(function(gapField){return gapField.latex()})}}:{base:{string:this.mathField.latex()}}},resetResponse:function(){var gapFields=this.getGapFields();this.inGapMode()?gapFields.forEach(function(gapField){gapField.latex("")}):this.setLatex("")},destroy:function(){this.$input.find(".mq-editable-field").off(ns),this.$input.off(ns),this.$toolbar.off(ns),this.resetResponse(),this.mathField instanceof MathQuill&&this.mathField.revert()},setSerializedState:function(state){this.setResponse(state)},getSerializedState:function(){return this.getResponse()}},qtiCustomInteractionContext.register(mathEntryInteraction);
}),define(["mathEntryInteraction/runtime/mathEntryInteraction"],function(PCI){return PCI});
