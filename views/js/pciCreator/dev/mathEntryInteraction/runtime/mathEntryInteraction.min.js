...
"undefined"!=typeof Map&&"function"==typeof(new Map).values&&(new Map).values().next||(e.Map=f({delete:d,has:q,get:m,set:r,keys:z,values:l,entries:A,forEach:u,clear:h})),"undefined"!=typeof Set&&"function"==typeof(new Set).values&&(new Set).values().next||(e.Set=f({has:p,add:t,delete:d,clear:h,keys:l,values:l,entries:B,forEach:u})),"undefined"==typeof WeakSet&&(e.WeakSet=f({delete:d,add:t,clear:h,has:p},!0))}("undefined"!=typeof exports&&"undefined"!=typeof global?global:window),define("mathEntryInteraction/runtime/polyfill/es6-collections",function(){}),
define("mathEntryInteraction/runtime/mathEntryInteraction",["qtiCustomInteractionContext","taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/OAT/util/html","mathEntryInteraction/runtime/mathquill/mathquill","mathEntryInteraction/runtime/polyfill/es6-collections"],function(qtiCustomInteractionContext,$,_,event,html,MathQuill){"use strict";function htmlMarkup(cls,tag){return"<"+(tag=tag||"div")+' class="'+cls+'"></'+tag+">"}function getWidth(element){
var style=element.currentStyle||window.getComputedStyle(element),rect=element.getBoundingClientRect(),borderBox="border-box"===style.boxSizing;return rect.width+parseFloat(style.marginLeft)+parseFloat(style.marginRight)+(borderBox?0:parseFloat(style.paddingLeft)+parseFloat(style.paddingRight))+(borderBox?0:parseFloat(style.borderLeftWidth)+parseFloat(style.borderRightWidth))}var mathEntryInteraction,ns=".mathEntryInteraction",cssClass={root:"mq-root-block",cursor:"mq-cursor",newLine:"mq-tao-br",autoWrap:"mq-tao-wrap"},cssSelectors=_.mapValues(cssClass,function(cls){return"."+cls}),reSpace=/^(\s|&nbsp;)+$/,MQ=MathQuill.getInterface(2)
;MQ.registerEmbed("gap",function(){return{htmlString:htmlMarkup("mq-tao-gap","span"),text:function(){return"tao_gap"},latex:function(){return"\\taoGap"}}}),MQ.registerEmbed("br",function(){return{htmlString:htmlMarkup(cssClass.newLine),text:function(){return"tao_br"},latex:function(){return"\\taoBr"}}}),mathEntryInteraction={inQtiCreator:function(){return _.isUndefined(this._inQtiCreator)&&this.$container&&(this._inQtiCreator=this.$container.hasClass("tao-qti-creator-context")),this._inQtiCreator},inGapMode:function(){return this.config.useGapExpression},render:function(config){this.initConfig(config),this.createToolbar(),this.togglePlaceholder(!1),
...
self.autoWrapContent()},enter:function(mathField){(function(){return self.config.allowNewLine&&(!self.inGapMode()&&!self.inQtiCreator()||self.inGapMode()&&self.inQtiCreator())})()&&mathField.write("\\embed{br}")}}}},togglePlaceholder:function(displayPlaceholder){this.$inputPlaceholder||(this.$inputPlaceholder=$("<div>",{
class:"math-entry-placeholder"}),this.$toolbar.after(this.$inputPlaceholder)),displayPlaceholder?(this.$input.hide(),this.$inputPlaceholder.show()):(this.$input.css({display:"block"}),this.$inputPlaceholder.hide())},autoWrapContent:function(){var $container,$cursor,current,lastSpace,lineBreak,maxWidth,lineWidth,cache,nodes,node,index,length,block;if(this.config.enableAutoWrap)for($container=this.$input.find(cssSelectors.root),$cursor=$container.find(cssSelectors.cursor),current=$cursor.closest(cssSelectors.root+">span").get(0),maxWidth=$container.width(),this.wrapCache||(this.wrapCache=new window.WeakMap),cache=this.wrapCache,lineWidth=0,
nodes=_.toArray($container.get(0).childNodes),length=nodes.length,index=0;index<length;index++)node=nodes[index],block=cache.get(node),block||(block={classList:node.classList,isSpace:reSpace.test(node.innerHTML)},cache.set(node,block)),block.classList.contains(cssClass.autoWrap)?$(node).remove():block.classList.contains(cssClass.newLine)?lineWidth=0:block.classList.contains(cssClass.cursor)||(current!==node&&void 0!==block.width||(block.width=getWidth(node)),block.isSpace&&(lastSpace={node:node,index:index}),lineWidth+block.width>=maxWidth&&(lineBreak=htmlMarkup(cssClass.autoWrap),lastSpace?($(lastSpace.node).after(lineBreak),index=lastSpace.index,
lineWidth=-block.width):($(node).before(lineBreak),lineWidth=0),lastSpace=null),lineWidth+=block.width)},setMathStaticContent:function(latex){latex=latex.replace(/\\taoGap/g,"\\MathQuillMathField{}").replace(/\\taoBr/g,"\\embed{br}"),this.$input.text(latex)},createMathStatic:function(){var gapFields,self=this;this.mathField=MQ.StaticMath(this.$input.get(0)),gapFields=this.getGapFields(),gapFields.forEach(function(field){field.config(self.getMqConfig())})},monitorActiveGapField:function(){var self=this,$editableFields=this.$input.find(".mq-editable-field");this._activeGapFieldIndex=null,$editableFields.length&&$editableFields.each(function(index){
$(this).off(ns).on("click"+ns+" keyup"+ns,function(){self._activeGapFieldIndex=index})})},createMathEditable:function(){var config=this.getMqConfig();this.mathField&&this.mathField instanceof MathQuill?this.mathField.config(config):this.mathField=MQ.MathField(this.$input.get(0),config)},setLatex:function(latex){var gapFields;this.inGapMode()&&_.isArray(latex)?(gapFields=this.getGapFields(),latex.forEach(function(latexExpr,i){gapFields[i]&&gapFields[i].latex(latexExpr)})):(latex=latex.replace(/\\taoGap/g,"\\embed{gap}").replace(/\\taoBr/g,"\\embed{br}").replace(/\\text\{\}/g,"\\text{ }"),this.mathField.latex(latex))},insertLatex:function(latex,fn){
var activeMathField=this.getActiveMathField();activeMathField&&_.isFunction(activeMathField[fn])&&(activeMathField[fn](latex),activeMathField.focus())},getActiveMathField:function(){var activeMathField,gapFields;return this.inGapMode()&&!this.inQtiCreator()?(_.isFinite(this._activeGapFieldIndex)||(this._activeGapFieldIndex=0),gapFields=this.getGapFields(),gapFields.length>0&&(activeMathField=gapFields[this._activeGapFieldIndex])):activeMathField=this.mathField,activeMathField},getGapFields:function(){return this.mathField&&_.isArray(this.mathField.innerFields)?this.mathField.innerFields:[]},addGap:function(){
this.inQtiCreator()&&this.insertLatex("\\embed{gap}","write")},createToolbar:function(){var self=this,availableTools={frac:{label:"x/y",latex:"\\frac",fn:"cmd",desc:"Fraction"},sqrt:{label:"&radic;",latex:"\\sqrt",fn:"cmd",desc:"Square root"},exp:{label:"x&#8319;",latex:"^",fn:"cmd",desc:"Exponent"},log:{label:"log",latex:"\\log",fn:"cmd",desc:"Log"},ln:{label:"ln",latex:"\\ln",fn:"cmd",desc:"Ln"},e:{label:"&#8494;",latex:"\\mathrm{e}",fn:"write",desc:"Euler's constant"},infinity:{label:"&#8734;",latex:"\\infty",fn:"cmd",desc:"Infinity"},lbrack:{label:"[",latex:"\\lbrack",fn:"cmd",desc:"Left bracket"},rbrack:{label:"]",
latex:"\\rbrack",fn:"cmd",desc:"Right bracket"},pi:{label:"&pi;",latex:"\\pi",fn:"cmd",desc:"Pi"},cos:{label:"cos",latex:"\\cos",fn:"cmd",desc:"Cosinus"},sin:{label:"sin",latex:"\\sin",fn:"cmd",desc:"Sinus"},lte:{label:"&le;",latex:"\\le",fn:"cmd",desc:"Lower than or equal"},gte:{label:"&ge;",latex:"\\ge",fn:"cmd",desc:"Greater than or equal"},times:{label:"&times;",latex:"\\times",fn:"cmd",desc:"Multiply"},divide:{label:"&divide;",latex:"\\div",fn:"cmd",desc:"Divide"},plusminus:{label:"&#177;",latex:"\\pm",fn:"cmd",desc:"Plus/minus"},angle:{label:"&ang;",latex:"\\angle",fn:"cmd",desc:"Angle"},minus:{label:"–",latex:"-",fn:"write",desc:"Minus"},plus:{
label:"+",latex:"+",fn:"write",desc:"Plus"},equal:{label:"=",latex:"=",fn:"write",desc:"Equal"},lower:{label:"<",latex:"<",fn:"write",desc:"Lower than"},greater:{label:">",latex:">",fn:"write",desc:"Greater than"},subscript:{label:"x&#8345;",latex:"_",fn:"cmd",desc:"Subscript"},lbrace:{label:"{",latex:"\{",fn:"cmd",desc:"Left brace/curly bracket"},rbrace:{label:"}",latex:"\}",fn:"cmd",desc:"Right brace/curly bracket"},lparen:{label:"(",latex:"(",fn:"write",desc:"Left parenthese/round bracket"},rparen:{label:")",latex:")",fn:"write",desc:"Right parenthese/round bracket"},integral:{label:"&#x222b;",latex:"\\int",fn:"cmd",desc:"Indefinitve integral"},timesdot:{
label:"·",latex:"\\cdot",fn:"cmd",desc:"Times dot"},triangle:{label:"&#9651;",latex:"\\triangle",fn:"cmd",desc:"Triangle"},similar:{label:"&sim;",latex:"\\sim",fn:"cmd",desc:"Similar"},paral:{label:"&#8741;",latex:"\\parallel",fn:"cmd",desc:"Is parallel with"},perp:{label:"&#8869;",latex:"\\perp",fn:"cmd",desc:"Is perpendicular to"},inmem:{label:"&isin;",latex:"\\in",fn:"cmd",desc:"Is member of"},ninmem:{label:"&notin;",latex:"\\notin",fn:"cmd",desc:"Is notmember of"},union:{label:"&cup;",latex:"\\cup",fn:"cmd",desc:"Set union"},intersec:{label:"&cap;",latex:"\\cap",fn:"cmd",desc:"Set intersection"}},availableToolGroups=[{id:"functions",tools:["sqrt","frac","exp","subscript","log","ln"]},
{id:"symbols",tools:["e","infinity","lparen","rparen","lbrace","rbrace","lbrack","rbrack","integral"]},{id:"geometry",tools:["angle","triangle","similar","paral","perp"]},{id:"trigo",tools:["pi","sin","cos"]},{id:"comparison",tools:["lower","greater","lte","gte"]},{id:"operands",tools:["equal","plus","minus","times","timesdot","divide","plusminus","inmem","ninmem","union","intersec"]}];this.$toolbar.empty(),availableToolGroups.forEach(function(toolgroup){self.$toolbar.append(self.createToolGroup(toolgroup,availableTools))})},createToolGroup:function(group,availableTools){var self=this,$toolGroup=$("<div>",{
class:"math-entry-toolgroup","data-identifier":group.id}),activeTools=0;return group.tools.forEach(function(toolId){var toolConfig=availableTools[toolId];toolConfig.id=toolId,!0===self.config.toolsStatus[toolId]&&($toolGroup.append(self.createTool(toolConfig)),activeTools++)}),activeTools>0?$toolGroup:""},createTool:function(config){return $("<div>",{class:"math-entry-tool","data-identifier":config.id,"data-latex":config.latex,"data-fn":config.fn,html:config.label})},addToolbarListeners:function(){var self=this;this.$toolbar.off("mousedown"+ns).on("mousedown"+ns,function(e){var $target=$(e.target),fn=$target.data("fn"),latex=$target.data("latex")
;e.stopPropagation(),e.preventDefault(),self.insertLatex(latex,fn)})},addGapStyle:function(){var self=this;self.config.gapStyle&&(self.$container.removeClass(function(index,className){return(className.match(/\bmath-gap-[\w]+\b/g)||[]).join(" ")}),self.$container.addClass(self.config.gapStyle))},id:-1,getTypeIdentifier:function(){return"mathEntryInteraction"},initialize:function(id,dom,config){var self=this;event.addEventMgr(this),this.id=id,this.dom=dom,this.$container=$(dom),this.$toolbar=this.$container.find(".toolbar"),this.$input=this.$container.find(".math-entry-input"),this.render(config),this.on("configChange",function(newConfig){
self.render(newConfig)}),this.on("addGap",function(){self.addGap()}),html.render(this.$container.find(".prompt")),qtiCustomInteractionContext.notifyReady(this)},setResponse:function(response){this.inGapMode()?response&&response.list&&_.isArray(response.list.string)&&this.setLatex(response.list.string):response&&response.base&&response.base.string&&this.setLatex(response.base.string)},getResponse:function(){return this.inGapMode()?{list:{string:this.getGapFields().map(function(gapField){return gapField.latex()})}}:{base:{string:this.mathField.latex()}}},resetResponse:function(){var gapFields=this.getGapFields()
;this.inGapMode()?gapFields.forEach(function(gapField){gapField.latex("")}):this.setLatex("")},destroy:function(){this.$input.find(".mq-editable-field").off(ns),this.$input.off(ns),this.$toolbar.off(ns),this.resetResponse(),this.mathField instanceof MathQuill&&this.mathField.revert()},setSerializedState:function(state){this.setResponse(state)},getSerializedState:function(){return this.getResponse()}},qtiCustomInteractionContext.register(mathEntryInteraction)}),define(["mathEntryInteraction/runtime/mathEntryInteraction"],function(PCI){return PCI});
