...
response:this.getResponse()}},_currentResponse:{base:null},_callbacks:[],_drawLiquidContainer:function(y){var canvas=$(this.dom).find(".liquids")[0],ctx=canvas.getContext("2d");ctx.lineWidth=2,ctx.strokeStyle="black",ctx.fillStyle="white",ctx.beginPath(),ctx.rect(210,20,160,280),ctx.stroke(),ctx.beginPath(),ctx.moveTo(210,20),ctx.lineTo(130,70),ctx.stroke(),ctx.beginPath(),ctx.moveTo(210,300),ctx.lineTo(130,350),ctx.stroke(),ctx.beginPath(),ctx.moveTo(370,300),ctx.lineTo(290,350),ctx.stroke(),void 0!==y&&this._drawLiquid(ctx,y),ctx.beginPath(),ctx.moveTo(370,20),ctx.lineTo(290,70),ctx.stroke(),ctx.strokeStyle="black",ctx.fillStyle="black",ctx.beginPath(),
ctx.rect(130,70,160,280),ctx.stroke(),this._drawGraduations(ctx,2,"16px Arial"),this._drawLabels(ctx,"16px Arial","3 1/4 in","4 in")},_drawGraduations:function(ctx,lineWidth,font){ctx.lineWidth=lineWidth,ctx.font=font,ctx.textAlign="right";var i=0,y=350;for(i;i<=10;i++)ctx.beginPath(),ctx.moveTo(120,y),ctx.lineTo(140,y),ctx.stroke(),ctx.fillText(i,110,y+5),y-=25},_drawLabels:function(ctx,font,xLabel,zLabel){ctx.font=font,ctx.textAlign="center",ctx.fillText(xLabel,210,370),ctx.fillText(zLabel,360,340)},_clearCanvas:function(){var canvas=$(this.dom).find(".liquids")[0];canvas.getContext("2d").clearRect(0,0,canvas.width,canvas.height)},
_isCoordInLiquidContainer:function(x,y){return x>=130&&x<=290&&y>=100&&y<=350},_drawLiquid:function(ctx,y){var closestY=this._closestMultiple(y,25);y=closestY,ctx.fillStyle=350!==closestY?"#003ADB":"transparent",ctx.beginPath(),ctx.moveTo(130,y),ctx.lineTo(290,y),ctx.lineTo(370,y-50),ctx.lineTo(210,y-50),ctx.lineTo(130,y),ctx.fill(),ctx.beginPath(),ctx.fillStyle="#4B6FD1",ctx.rect(130,y,160,350-y),ctx.fill(),ctx.beginPath(),ctx.fillStyle="#7B96E3",ctx.moveTo(290,y),ctx.lineTo(369,y-50),ctx.lineTo(369,y+(300-y)),ctx.lineTo(290,y+(350-y)),ctx.fill()},_closestMultiple:function(numeric,multiple){return multiple*Math.floor((numeric+multiple/2)/multiple)},
_yFromResponse:function(response){return null===response.base?350:350-this._closestMultiple(25*response.base.integer,25)},_updateResponse:function(y){this._currentResponse={base:{integer:Math.abs((this._closestMultiple(y,25)-350)/25)}}}};qtiCustomInteractionContext.register(liquidsInteraction)}),define(["liquidsInteraction/runtime/liquidInteraction"],function(PCI){return PCI});
