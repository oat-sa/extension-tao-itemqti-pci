...
e.callback.apply({type:event,ns:[]},data)})}}}),define("taoQtiItem/portableLib/OAT/util/event",["taoQtiItem/portableLib/OAT/util/EventMgr"],function(EventMgr){return{addEventMgr:function(instance){var eventMgr=new EventMgr;instance.on=function(event,callback){eventMgr.on(event,callback)},instance.off=function(event){eventMgr.off(event)},instance.trigger=function(event,data){eventMgr.trigger(event,data)}}}}),define("liquidsInteraction/runtime/liquidInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/OAT/util/event"],function($,qtiCustomInteractionContext,event){var liquidsInteraction={id:-1,getTypeIdentifier:function(){
return"liquidsInteraction"},initialize:function(id,dom,config){event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config||{};var canvas=$(this.dom).find(".liquids")[0];this._drawLiquidContainer(),qtiCustomInteractionContext.notifyReady(this);var self=this;$(canvas).on("click",function(e){var rect=canvas.getBoundingClientRect(),x=e.clientX-rect.left,y=e.clientY-rect.top;self._isCoordInLiquidContainer(x,y)===!0&&(self._clearCanvas(),self._drawLiquidContainer(y),self._updateResponse(y),self.trigger("responsechange",[self.getResponse()]))})},setResponse:function(response){var y=this._yFromResponse(response);this._clearCanvas(),this._drawLiquidContainer(y),
this._updateResponse(y)},getResponse:function(){return this._currentResponse},resetResponse:function(){this._clearCanvas(),this._drawLiquidContainer(),this._currentResponse={base:null}},destroy:function(){var $container=$(this.dom);$container.find(".canvas").off()},setSerializedState:function(state){},getSerializedState:function(){return{}},_currentResponse:{base:null},_callbacks:[],_drawLiquidContainer:function(y){var canvas=$(this.dom).find(".liquids")[0],ctx=canvas.getContext("2d"),lineWidth=2,font="16px Arial",xLabel="3 1/4 in",zLabel="4 in";ctx.lineWidth=lineWidth,ctx.strokeStyle="black",ctx.fillStyle="white",ctx.beginPath(),ctx.rect(210,20,160,280),ctx.stroke(),
ctx.beginPath(),ctx.moveTo(210,20),ctx.lineTo(130,70),ctx.stroke(),ctx.beginPath(),ctx.moveTo(210,300),ctx.lineTo(130,350),ctx.stroke(),ctx.beginPath(),ctx.moveTo(370,300),ctx.lineTo(290,350),ctx.stroke(),"undefined"!=typeof y&&this._drawLiquid(ctx,y),ctx.beginPath(),ctx.moveTo(370,20),ctx.lineTo(290,70),ctx.stroke(),ctx.strokeStyle="black",ctx.fillStyle="black",ctx.beginPath(),ctx.rect(130,70,160,280),ctx.stroke(),this._drawGraduations(ctx,lineWidth,font),this._drawLabels(ctx,font,xLabel,zLabel)},_drawGraduations:function(ctx,lineWidth,font){ctx.lineWidth=lineWidth,ctx.font=font,ctx.textAlign="right";var i=0,x=130,y=350,space=25,width=20;for(i;i<=10;i++)ctx.beginPath(),
ctx.moveTo(x-width/2,y),ctx.lineTo(x+width/2,y),ctx.stroke(),ctx.fillText(i,x-width,y+5),y-=space},_drawLabels:function(ctx,font,xLabel,zLabel){ctx.font=font,ctx.textAlign="center",ctx.fillText(xLabel,210,370),ctx.fillText(zLabel,360,340)},_clearCanvas:function(){var canvas=$(this.dom).find(".liquids")[0],ctx=canvas.getContext("2d");ctx.clearRect(0,0,canvas.width,canvas.height)},_isCoordInLiquidContainer:function(x,y){return x>=130&&x<=290&&y>=100&&y<=350},_drawLiquid:function(ctx,y){var closestY=this._closestMultiple(y,25);y=closestY,ctx.fillStyle=350!==closestY?"#003ADB":"transparent",ctx.beginPath(),ctx.moveTo(130,y),ctx.lineTo(290,y),ctx.lineTo(370,y-50),
ctx.lineTo(210,y-50),ctx.lineTo(130,y),ctx.fill(),ctx.beginPath(),ctx.fillStyle="#4B6FD1",ctx.rect(130,y,160,350-y),ctx.fill(),ctx.beginPath(),ctx.fillStyle="#7B96E3",ctx.moveTo(290,y),ctx.lineTo(369,y-50),ctx.lineTo(369,y+(300-y)),ctx.lineTo(290,y+(350-y)),ctx.fill()},_closestMultiple:function(numeric,multiple){return multiple*Math.floor((numeric+multiple/2)/multiple)},_yFromResponse:function(response){return null===response.base?350:350-this._closestMultiple(25*response.base.integer,25)},_updateResponse:function(y){this._currentResponse={base:{integer:Math.abs((this._closestMultiple(y,25)-350)/25)}}}};qtiCustomInteractionContext.register(liquidsInteraction);
}),define(["liquidsInteraction/runtime/liquidInteraction"],function(PCI){return PCI});
//# sourceMappingURL=liquidInteraction.min.js.map