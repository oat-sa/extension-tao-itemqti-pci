...
define("liquidsInteraction/runtime/liquidInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/OAT/util/event"],function($,qtiCustomInteractionContext,event){var liquidsInteraction={id:-1,getTypeIdentifier:function(){return"liquidsInteraction"},initialize:function(id,dom,config){event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config||{};var canvas=$(this.dom).find(".liquids")[0];this._drawLiquidContainer(),qtiCustomInteractionContext.notifyReady(this);var self=this;$(canvas).on("click",function(e){var rect=canvas.getBoundingClientRect(),x=e.clientX-rect.left,y=e.clientY-rect.top
;!0===self._isCoordInLiquidContainer(x,y)&&(self._clearCanvas(),self._drawLiquidContainer(y),self._updateResponse(y),self.trigger("responsechange",[self.getResponse()]))})},setResponse:function(response){var y=this._yFromResponse(response);this._clearCanvas(),this._drawLiquidContainer(y),this._updateResponse(y)},getResponse:function(){return this._currentResponse},resetResponse:function(){this._clearCanvas(),this._drawLiquidContainer(),this._currentResponse={base:null}},destroy:function(){$(this.dom).find(".canvas").off()},setSerializedState:function(state){state&&state.response&&this.setResponse(state.response)},getSerializedState:function(){return{
response:this.getResponse()}},_currentResponse:{base:null},_callbacks:[],_drawLiquidContainer:function(y){var canvas=$(this.dom).find(".liquids")[0],ctx=canvas.getContext("2d");ctx.lineWidth=2,ctx.direction="ltr",ctx.strokeStyle="black",ctx.fillStyle="white",ctx.beginPath(),ctx.rect(210,20,160,280),ctx.stroke(),ctx.beginPath(),ctx.moveTo(210,20),ctx.lineTo(130,70),ctx.stroke(),ctx.beginPath(),ctx.moveTo(210,300),ctx.lineTo(130,350),ctx.stroke(),ctx.beginPath(),ctx.moveTo(370,300),ctx.lineTo(290,350),ctx.stroke(),void 0!==y&&this._drawLiquid(ctx,y),ctx.beginPath(),ctx.moveTo(370,20),ctx.lineTo(290,70),ctx.stroke(),ctx.strokeStyle="black",
ctx.fillStyle="black",ctx.beginPath(),ctx.rect(130,70,160,280),ctx.stroke(),this._drawGraduations(ctx,2,"16px Arial"),this._drawLabels(ctx,"16px Arial","3 1/4 in","4 in")},_drawGraduations:function(ctx,lineWidth,font){ctx.lineWidth=lineWidth,ctx.font=font,ctx.textAlign="right";var i=0,y=350;for(i;i<=10;i++)ctx.beginPath(),ctx.moveTo(120,y),ctx.lineTo(140,y),ctx.stroke(),ctx.fillText(i,110,y+5),y-=25},_drawLabels:function(ctx,font,xLabel,zLabel){ctx.font=font,ctx.textAlign="center",ctx.fillText(xLabel,210,370),ctx.fillText(zLabel,360,340)},_clearCanvas:function(){var canvas=$(this.dom).find(".liquids")[0]
;canvas.getContext("2d").clearRect(0,0,canvas.width,canvas.height)},_isCoordInLiquidContainer:function(x,y){return x>=130&&x<=290&&y>=100&&y<=350},_drawLiquid:function(ctx,y){var closestY=this._closestMultiple(y,25);y=closestY,ctx.fillStyle=350!==closestY?"#003ADB":"transparent",ctx.beginPath(),ctx.moveTo(130,y),ctx.lineTo(290,y),ctx.lineTo(370,y-50),ctx.lineTo(210,y-50),ctx.lineTo(130,y),ctx.fill(),ctx.beginPath(),ctx.fillStyle="#4B6FD1",ctx.rect(130,y,160,350-y),ctx.fill(),ctx.beginPath(),ctx.fillStyle="#7B96E3",ctx.moveTo(290,y),ctx.lineTo(369,y-50),ctx.lineTo(369,y+(300-y)),ctx.lineTo(290,y+(350-y)),ctx.fill()},
_closestMultiple:function(numeric,multiple){return multiple*Math.floor((numeric+multiple/2)/multiple)},_yFromResponse:function(response){return null===response.base?350:350-this._closestMultiple(25*response.base.integer,25)},_updateResponse:function(y){this._currentResponse={base:{integer:Math.abs((this._closestMultiple(y,25)-350)/25)}}}};qtiCustomInteractionContext.register(liquidsInteraction)}),define(["liquidsInteraction/runtime/liquidInteraction"],function(PCI){return PCI});
