...
}).html(config["label-min"]),$labelMax=$("<span>",{class:"likert-label likert-label-max"}).html(config["label-max"]);$labelMin.append($("<img>",{src:assetManager.resolve("likertScaleInteraction/runtime/assets/ThumbDown.png")}).css({top:6,marginLeft:12})),$labelMax.prepend($("<img>",{src:assetManager.resolve("likertScaleInteraction/runtime/assets/ThumbUp.png")}).css({top:2,marginRight:12})),$ul.before($labelMin),$ul.after($labelMax)}return{render:function(id,container,config,assetManager){var $container=$(container);renderChoices(id,$container,config),renderLabels(id,$container,config,assetManager),html.render($container.find(".prompt"))},
