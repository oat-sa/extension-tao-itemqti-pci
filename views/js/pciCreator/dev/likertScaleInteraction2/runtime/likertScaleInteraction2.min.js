...
}).html(config["label-min"]),$labelMax=$("<span>",{class:"likert-label likert-label-max"}).html(config["label-max"]);$labelMin.append($("<img>",{src:assetManager.resolve("likertScaleInteraction/runtime/assets/ThumbDown.png"),class:"likert-image-append"})),$labelMax.prepend($("<img>",{src:assetManager.resolve("likertScaleInteraction/runtime/assets/ThumbUp.png"),class:"likert-image-prepend"})),$ul.before($labelMin),$ul.after($labelMax)}return{render:function(id,container,config,assetManager){var $container=$(container);renderChoices(id,$container,config),renderLabels(id,$container,config,assetManager),html.render($container.find(".prompt"))},
...
state&&state.response&&this.setResponse(state.response)},getSerializedState:function(){return{response:this.getResponse()}}};qtiCustomInteractionContext.register(likertScaleInteraction)}),define(["likertScaleInteraction/runtime/js/likertScaleInteraction"],function(PCI){return PCI});
