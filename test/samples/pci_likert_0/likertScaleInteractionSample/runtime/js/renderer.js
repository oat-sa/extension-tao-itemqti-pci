...
define(['jquery_2_1_1'], function($){
    ...
    function renderLabels(id, $container, config, assetManager){
        ...
        if(assetManager){
            $labelMin.append($('<img>', {src: assetManager.resolve('likertScaleInteractionSample/runtime/assets/ThumbDown.png')}).css({top: 6, marginLeft:12}));
            $labelMax.prepend($('<img>', {src: assetManager.resolve('likertScaleInteractionSample/runtime/assets/ThumbUp.png')}).css({top: 2, marginRight:12}));
        }else{
            $labelMin.append($('<img>', {src: $container.find('.assets .ThumbDown').attr('src')}).css({top: 6, marginLeft:12}));
            $labelMax.prepend($('<img>', {src: $container.find('.assets .ThumbUp').attr('src')}).css({top: 2, marginRight:12}));
        }
