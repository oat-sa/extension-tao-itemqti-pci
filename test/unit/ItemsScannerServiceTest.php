...
class ItemsScannerServiceTest extends GenerisTestCase
{
     ...
     * @test
     */
    public function howLongDoesItTakes()
    {
        $qtiService = QtiService::singleton();
        $scaznner = new ItemsScannerService($qtiService);
        $allItems = \taoTests_models_classes_TestsService::singleton()->getAllItems();
        var_dump(count($allItems));
        $scaznner->isPciUsedInItems('someString', $allItems);
    }
}
